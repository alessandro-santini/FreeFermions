...
class sudden_quench:
    ...
    def time_evolve(self,t: float):
        self.wt = np.einsum('ij,j,jk,kl->il',self.W1,np.exp(-1j*2.*self.eigs1*t),self.W1.T.conj(),self.w0,optimization='greedy')#self.W1 @ expm(-1j*2.*np.diag(self.eigs1)*t) @ self.W1.T.conj() @ self.w0
